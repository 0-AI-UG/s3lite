...
interface VectorStoreInterface {
  ...
  _getAllIndexEntries(): Map<string, {
    ...
    hnsw: {
      ...
      allNodes(): IterableIterator<{ key: string; vector: Float32Array; metadata?: Record<string, unknown> }>;
      getNode(key: string): { key: string; vector: Float32Array; metadata?: Record<string, unknown> } | undefined;
    };
...
export class VectorWAL {
  ...
  open(store: VectorStoreInterface): void {
      ...
      if (buf.byteLength >= 8) {
        ...
        if (magicOk) {
          ...
          if (version === VECTOR_FORMAT_VERSION || version === 1) {
            ...
            if (hasGraph) {
              // Phase 1: Apply records, skip HNSW inserts, collect dense vectors
              ...
              // Phase 2: Deserialize HNSW graphs from snapshot
    ...
    // Replay WAL records normally (post-checkpoint ops, always with HNSW inserts)
  ...
  private deserializeGraphs(
    ...
    vectorsMap: Map<string, Map<string, { vector: Float32Array; norm: number; metadata?: Record<string, unknown> }>>,
  ): void {
    const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
    const decoder = new TextDecoder();
